...
st.title("🚀 Explosions v7.1 — Pre-Rally Scanner (BALANCED+)")
...
def get_info_fields(symbol: str):
    # Only used for top-N, to reduce slowness / missing value chaos
    try:
        t = yf.Ticker(symbol)
        info = t.info or {}
...
st.caption(
    "BALANCED+ = mehr Signale: Trend+RS dominieren, NearHigh/ATR lockerer. "
    "🟡 PRE-BREAKOUT ist Starter-Position / frühes Setup, 🟢 BUY erst bei Breakout+Volumen."
)

if st.button("🎯 FORCE SCAN v7.1 (Batch Download, balanced+)", type="primary"):
    st.cache_data.clear()
    bar = st.progress(0)
    ...
    for i, sym in enumerate(CANDIDATES):
        ...
        if df is None or len(df) < 80:
            results.append({
                ...
                "Trend": "—", "NearHigh_%": np.nan, "ATR%": np.nan,
                "VolDryUp": np.nan, "RS_3M": np.nan,
        ...
        sma50_slope = (
            safe_div((sma50.iloc[-1] - sma50.iloc[-21]), sma50.iloc[-21]) * 100.0
            if len(sma50.dropna()) > 60 else np.nan
        )
        slope_ok = np.isfinite(sma50_slope) and sma50_slope > 0

        # Relaxed trend label: ⚠️ if at least one of the two is ok
        ...
        near_high = np.nanmin([dist20, dist55])
        ...
        voldry = safe_div(vol5, vol20)  # <1 = drying up
        ...
        # Breakout confirmed?
        breakout20 = close_now > high20 * 1.001
        ...
        # --- sub scores 0..1 ---
        # Trend: relaxed, still meaningful
        trend_sub = 0.0
        if trend_ok:
            trend_sub += 0.6
        if slope_ok:
        ...
        # Near-high: relaxed mapping 0..15% => 1..0
        if np.isfinite(near_high):
            near_sub = clip01(1.0 - (max(near_high, 0.0) / 15.0))
        else:
            near_sub = 0.0

        # ATR%: relaxed mapping 2..7% => 1..0
        if np.isfinite(atr_pct):
            atr_sub = clip01(1.0 - ((atr_pct - 2.0) / 5.0))
        else:
            atr_sub = 0.0

        # Dry-up: voldry 0.6 => good, 1.0 => neutral
        if np.isfinite(voldry):
            dry_sub = clip01((1.0 - voldry) / 0.4)  # 0.6 -> 1.0, 1.0 -> 0
        else:
            dry_sub = 0.0

        results.append({
            "Symbol": sym,
            "Name": sym,     # fill later for topN
            "Sector": "N/A",
            "Trend": trend_label,
    ...
    # RS percentile within candidates
    df["_rs_sub"] = percentile_rank(df["RS_3M"])

    # SetupScore weights tuned for "more signals" but still sane:
    # Trend 30, RS 25, Near 20, ATR 15, Dry 10 (0..100)
    df["SetupScore"] = (
        df["_trend_sub"] * 30.0 +
        df["_rs_sub"]    * 25.0 +
        df["_near_sub"]  * 20.0 +
    ...
    # Catalyst (0..30), only for top N to avoid info slowness
    df["Catalyst"] = 0.0
    TOPN_INFO = 5
    ...
    # Timing logic (more signals, still balanced)
    timing = []
    entry_type = []
    for _, row in df.iterrows():
        ...
        if brk:
            ...
            # BALANCED+ thresholds (more permissive)
            pre_ok = (
                (setup >= 60) and
                (np.isfinite(near) and near <= 6.0) and
                (np.isfinite(atrp) and atrp <= 4.5) and
                trend_not_bad
            )
            watch_ok = (setup >= 50)
    ...
    # Summary
    signals = df["Timing"].str.contains("BUY|PRE-BREAKOUT|WATCH", na=False).sum()
    buy_pre = df["Timing"].str.contains("BUY|PRE-BREAKOUT", na=False).sum()

    st.success(f"✅ {len(df)}/{len(CANDIDATES)} gescannt | {buy_pre}x (BUY/PRE) | {signals}x (BUY/PRE/WATCH)")
    ...
    col3.metric("🟡 PRE-BREAKOUT", int(df["Timing"].str.contains("PRE-BREAKOUT", na=False).sum()))
    col4.metric("🟢 BUY (confirmed)", int(df["Timing"].str.contains("confirmed breakout", na=False).sum()))

    # Buckets
    st.subheader("🟡 PRE-BREAKOUT (Starter-Entries)")
    pre = df[df["Timing"].str.contains("PRE-BREAKOUT", na=False)]
    st.dataframe(pre[show_cols], use_container_width=True)

    st.subheader("🟢 BUY (Breakout confirmed + Volume)")
    brk = df[df["Timing"].str.contains("confirmed breakout", na=False)]
...
st.caption("v7.1 balanced+ | schneller Batch-Download | Debug-Subs zeigen dir, welcher Filter killt.")
